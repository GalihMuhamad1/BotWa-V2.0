...
ban = []
limitawal = "999999999"
gcounttprem = "999999999" 
gcounttuser = "999999999" 
...
img = setting.img
baper = 'Subscribe YouTube Galih Muhamad A'
apiku = 'https://youtu.be/D6fyC7O5oZA'
gc_wa_lu = 'https://wa.me/6281222180225text=KamuGanteng' //klo gk punya gc wa gk usah di ganti 👍
...
await alpha.updatePresence(from, Presence.composing)
		...
		try {
					...
					pp_userb = 'https://a.top4top.io/p_2158wzxsa0.jpg'
				}
	...
	if (position !== false) {
		...
		fs.writeFileSync('./src/level.json', JSON.stringify(_level))
					...
					const addLevelingId = (sender) => {
						const obj = { id: sender, xp: 999999, level: 1 }
						_level.push(obj)
						fs.writeFileSync('./src/level.json', JSON.stringify(_level))
						}
					const nebal = (angka) => {
					...
					function randomNomor(angka){
						return Math.floor(Math.random() * angka) + 99999
						}
					const levelRole = getLevelingLevel(sender)
					const addATM = (sender) => {
        	const objo = {id: sender, uang : 99999}
            uang.push(objo)
...
return stone[position].id
    ...
    const addBatuId = (sender) => {
        const ovj = {id: sender, batu: 999999}
        stone.push(ovj)
        fs.writeFileSync('./database/user/stone.json', JSON.stringify(stone))
        }
...
return ore[position].id
    ...
    const addOreId = (sender) => {
        const ovj = {id: sender, copperore: 123456789}
        ore.push(ovj)
        fs.writeFileSync('./database/user/ore.json', JSON.stringify(ore))
        }
...
return kayu[position].id
    ...
    const addNebangId = (sender) => {
        const ovj = {id: sender, wood: 100000}
        kayu.push(ovj)
        fs.writeFileSync('./database/user/kayu.json', JSON.stringify(kayu))
        }
...
const bayarLimit = (sender, amount) => {
	...
	//FUCNTION GOPLANET
	   ...
	   //FUCNTION mining coal
        ...
        if(time2 < "23:59:00"){
        var ucapannya2 = `Tidur Woy Sudah Malam-_- ${pushname}`
}
        if(time2 < "19:00:00"){
        ...
        if(time2 < "15:00:00"){
        var ucapannya2 = `Siangg:) 🌞 ${pushname}`
}
        if(time2 < "11:00:00"){
        var ucapannya2 = `Skrng Mah Enak Nya Ngopi ${pushname}`
}
        if(time2 < "05:00:00"){
        var ucapannya2 = `Subuh² Gini 🙄 ${pushname}`
}
...
if (autobio === false) return           
    ...
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await alpha.setStatus(`Owner Saya Galih Muhamad No WA:+6281222180225 | Run Time ${uptimer} ⏲️ | ${status}`).catch((_) => _);
      settingstatus = new Date() * 1;
    }}
		mess = {
			wait: '```[ ! ] Wajib Panggil Saya Galih Ganteng😂...```',
			success: '```[ ✓ ]``` Udah Tuh',
		...
		message: {
			"productMessage": {
				"product": {
					...
					"retailerId": 'Whatsapp bot By Galih M',
					"productImageCount": 1
				},
...
let id_vote = sender ? sender : '6281222180225@s.whatsapp.net'
...
const checkWin = (sender) => {
        ...
        orangnye = sender
        teks = `@${orangnye.split("@")[0]} NyerahAnkCulun${enter}_YahahaWahyu_`
  ...
  //FUNCTION
            ...
            alpha.sendMessage(mek.key.remoteJid,`@${owner} Ganteng:v. Sedang Off\n\n*Alasan Na Teh :* ${alasan}\n*Tipas Ker:* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik lalu\n\nKe Hubungi Deui Nyaa Geulis/Ganteng`, MessageType.text,{contextInfo:{ mentionedJid: [`${owner}@s.whatsapp.net`],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": "*Mnh Goreng Patut:v*", 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}})
            }
            }   
        if (mek.key.remoteJid.endsWith('@g.us') && offline) {
        if (!mek.key.fromMe){
    ...
    if (isGroup && isRegister && !isCmd) {
						...
						try {
							...
							if (requiredXp <= getLevelingXp(sender)) {
								...
								addBalance(sender, 99999, balance)
...
if (tebakgambar.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
                ...
                if (budy.toLowerCase() == jawaban) {
                    ...
                    reply2("Paan? Jawaban Lu Salah! Ego")
                }
            }
// CAK LONTONG
...
//========================================================================================================================//
		...
		if (isCmd && isGroup){
		console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32m CMD \x1b[1;37m]', timuu, color(command), 'from', color(pushname), 'in', color(groupName), 'args :', color(args.length))
		addBalance(sender, randomNomor(1000), balance)
			}	
            //await alpha.updatePresence(from, Presence.recording)
            //await alpha.updatePresence(from, Presence.composing)
...
case 'dompet':
	...
	case 'transfer':
				if (!q.includes('|')) return  reply('format salah')
                			...
                			if (jumblah < 10000 ) return reply(`Minimal Transfer 10000`)
...
*Example :*
${prefix}buylimit 999999
${prefix}buyglimit 999999

NOTE : 
- Harga Limit Per 999999 adalah $100 balance`)
break
case 'buylimit':{
if (!q) return reply(`Kirim perintah *${prefix}buylimit* jumlah limit yang ingin dibeli\n\nHarga 999999 limit = $100 balance`)
if (q.includes('-')) return reply(`Jangan menggunakan -`)
if (isNaN(q)) return reply(`Harus berupa angka`)
let ane = Number(nebal(q) * 1)
...
let papako = [{
	...
	case 'menu': case 'help':
			try {
				...
				chatt = 'https://a.top4top.io/p_2158wzxsa0.jpg'
				}
...
`
if(menusimple == false){
sendButDoc(from, ini_anu, `_Jangan Spam Galih Yah:), Tunggu 3 Detik per command._\n_Makasih!_`, sender, koko, ini_mark)
...
const buttons1 = [
  {buttonId: 'ownerku', buttonText: {displayText: ' Jienan Saha?'}, type: 1},
...
case 'bcgif':
         ...
         ini_ownerNumber = [`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`]
					...
					ini_list.push({
					"displayName": 'Owner Galih Muhamad',
...
case 'setexif':
		    ...
		    case 'take':
		    case 'colong':
		    		if (!isQuotedSticker) return reply('```Reply Stiker  na Ganteng/Geulis```')
		            ...
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `GALIH MUHAMAD A`
...
break
		...
		if (!mek.key.fromMe && !isGroupAdmins && !isOwner && !isCreator) return reply(lang.onlyOwner())
		     if (!isGroup) return fakegroup(lang.onlygc())
				...
				case 'listprem': case 'listpremium':
							...
							break
				case 'payment':
				let paykenekk = await getBuffer('https://a.top4top.io/p_2158wzxsa0.jpg')
				...
				case 'belipremium': case 'buypremium': case 'sewabot': case 'goprem':
							let sewalak = await getBuffer('https://a.top4top.io/p_2158wzxsa0.jpg')
				...
				case 'donasi': case 'donate':
							let donas = await getBuffer('https://a.top4top.io/p_2158wzxsa0.jpg')
				...
				case 'gcbot': case 'grupbot': case 'groupbot':
							let gcbot = await getBuffer('https://a.top4top.io/p_2158wzxsa0.jpg')
							...
							break
				case 'infobot':
				let infobopot = await getBuffer('https://a.top4top.io/p_2158wzxsa0.jpg')
		...
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					fakethumb(buffer,'```Nih Tod, Tong Poho Subs YT : GalihMuhamad A```')
					fs.unlinkSync(ran)
					})
					break
...
break
		    ...
		    case 'brainly':
					...
					teks += `\n*「 _JAWABAN DARI GALIH  ADALAH_ 」*\n\n*➸ Pertanyaan nya:* ${Y.pertanyaan}\n\n*➸ Jawaban Nya:* ${Y.jawaban[0].text}\n❉──────────────────❉\n`
					}
...
*Chapter Akhir* : ${komik.chapter_akhir}`
			...
			case 'term':
					...
					if (!q) return fakegroup(mess.wrongFormat)
					exec(q, (err, stdout) => {
					if (err) return fakegroup(`GALIH MUHAMAD:~ ${err}`)
					if (stdout) {
					fakegroup(stdout)
					}
					})
        ...
        break;
           ...
           case 'getpp':
				if (mek.message.extendedTextMessage != undefined){
					...
					anuu  = {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${numbernye}@s.whatsapp.net`, 'remoteJid': '6289523258649-1604595598@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 Bot by Galih Muhamad 」`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}}
...
sendButImage(from,  `Udh Jadi Yahh Jangan Lupa Makan Biar Gak Cepet Mati? ` , `${botname}™© | By ${ownername}`,photooxy2, [{"buttonId": `owner`,"buttonText": {"displayText": "👨‍💻 Owner"},"type": "RESPONSE"},{"buttonId": `donasi`,"buttonText": {"displayText": "💰 Donate"},"type": "RESPONSE"}], {thumbnail: Buffer.alloc(0), quoted: mek})
await  limitAdd(sender, limit)
break         
...
sendButImage(from,  `Udh Jadi Yahh Jangan Lupa Makan Biar Gak Cepet Mati ` , `${botname}™© | By ${ownername}`,textpro, [{"buttonId": `owner`,"buttonText": {"displayText": "👨‍💻 Owner"},"type": "RESPONSE"},{"buttonId": `donasi`,"buttonText": {"displayText": "💰 Donate"},"type": "RESPONSE"}], {thumbnail: Buffer.alloc(0), quoted: mek})
await  limitAdd(sender, limit)
break 
//1 TEXT  
case 'blackpink':
...
sendButImage(from,  `Udh Jadi Yahh Jangan Lupa Makan Biar Gak Cepet Mati ` , `${botname}™© | By ${ownername}`,textpro2, [{"buttonId": `owner`,"buttonText": {"displayText": "👨‍💻 Owner"},"type": "RESPONSE"},{"buttonId": `donasi`,"buttonText": {"displayText": "💰 Donate"},"type": "RESPONSE"}], {thumbnail: Buffer.alloc(0), quoted: mek})
await  limitAdd(sender, limit)
break  
case 'tahta':  
                   if (args.length < 1) return reply(lang.noteks(prefix, command))
                   ...
                   tahtah = `${lang.success()}\n\nPlease Subscribe https://youtu.be/D6fyC7O5oZA`
       ...
       case 'ttp':  
         ...
         case 'attp':  
             ...
             case 'ktpmaker': case 'ktp':
                    if (args.length == 0) return reply(`Usage: ${prefix + command} nik|provinsi|kabupaten|nama|tempat, tanggal lahir|jenis kelamin|jalan|rt/rw|kelurahan|kecamatan|agama|status nikah|pekerjaan|warga negara|berlaku sampai|url_image\n\nExample: ${prefix + command} 456127893132123|bumipertiwi|fatamorgana|LoL Human|mars, 99-99-9999|belum ditemukan|jl wardoyo|999/999|turese|imtuni|alhamdulillah islam|jomblo kack|mikirin dia|indo ori no kw|hari kiamat|https://a.top4top.io/p_2158wzxsa0.jpg`)
   ...
   break
    ...
    case 'voting':
		...
		case 'jadibot':
			...
			var jadibdj = await getBuffer('https://a.top4top.io/p_2158wzxsa0.jpg')
...
console.log(command + '  -> Mungkin fitur ini masih suka eror ngab jadi fix sendiri ya')
     ...
     if (isQuotedImage) {
     reply(lang.wait())
     let ppk = await alpha.getProfilePicture(sender).catch(_ => 'https://a.top4top.io/p_2158wzxsa0.jpg')
...
break
 ...
 members_ids = []
					...
					ini_list.push({
					"displayName": 'Owner GalihM.A',
					"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname_ ? `${vname_}` : `${alpha.user.name}`}\nORG: SUBSCRIBE Galih Muhamad;\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
					})
					}
					hehe = await alpha.sendMessage(from, {
					"displayName": `${ini_list.length} kontak`,
...
if (subscribeGalih Muhamad A == 'statiktiknya') {
...
if (subscribezeeoneofc == "ownerku"){
					...
					const vname_ = alpha.contacts[i] != undefined ? alpha.contacts[i].vname || alpha.contacts[i].notify : undefined
					ini_list.push({
					"displayName": 'Owner GalihMuhamad',
					"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname_ ? `${vname_}` : `${alpha.user.name}`}\nORG: Galih Muhamad;\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
					})
					}
					hehe = await alpha.sendMessage(from, {
					...
					var ini_gopayy =`Halo @${sender.split("@")[0]} itu owner Aing, Dek Naon?`
var buttonss = [
{buttonId: 'donasi', buttonText:{displayText: 'Donasi'}, type: 1},
{buttonId: 'Menu', buttonText:{displayText: 'MENU'}, type: 1}
]
...
if (budy.includes("eror",'error','Eror','Error')){
					...
					alpha.sendMessage(from, daieeeee, sticker, { contextInfo: {mentionedJid: [sender]}, quoted: { "key": { "participant": `${numbernye}@s.whatsapp.net`, "remoteJid":  '6283136505591-1614953337@g.us', "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": fs.readFileSync(`image/${thumbnail}`), "mimetype": "application/octet-stream","title": "YT : Galih Muhamad", "fileLength": "36", "pageCount": 0, "fileName": `Kenapa bisa error???`}}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		        }
		if(budy.includes("@verif", "@verify","daftar")){
			if (isRegister) return reply('Kamu sudah terdaftar di dalam database')
 addRegisterUser(sender, pushname, bio_user, wib)
 let ran_blc = randomNomor(999999999)
 addBalance(sender, ran_blc, balance)
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`GalihM.A :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
